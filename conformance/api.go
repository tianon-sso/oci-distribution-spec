...
func (a *api) getAuthHeader(client http.Client, resp *http.Response) (string, error) {
	cacheKey, err := authCacheKey(*resp.Request)
	if err != nil {
		return "", err
	}
	...
	if parsed.Type == "basic" {
		auth := fmt.Sprintf("Basic %s", base64.StdEncoding.EncodeToString([]byte(a.user+":"+a.pass)))
		if a.authCache != nil {
			a.authCache[cacheKey] = auth
		}
		return auth, nil
	}
	if parsed.Type == "bearer" {
		...
		auth := fmt.Sprintf("Bearer %s", ai.Token)
		if a.authCache != nil {
			a.authCache[cacheKey] = auth
		}
		return auth, nil
...
func (a *api) addCachedAuth(req *http.Request) error {
	if a.authCache == nil {
		return nil
	}
	key, err := authCacheKey(*req)
	if err != nil {
		return err
	}
	if auth, ok := a.authCache[key]; ok {
		req.Header.Set("Authorization", auth)
	}
	return nil
}
...
var reRepo = regexp.MustCompile(`\/v2\/` +
	`([a-z0-9]+(?:(?:\.|_|__|-+)[a-z0-9]+)*(?:\/[a-z0-9]+(?:(?:\.|_|__|-+)[a-z0-9]+)*)*)` +
	`(\/(?:blobs|manifests|tags|referrers)\/(?:[^\/]+)|\/blobs\/uploads\/[^\/]*)`)

func authCacheKey(req http.Request) (string, error) {
	if req.URL == nil {
		return "", fmt.Errorf("cannot compute auth cache key, URL is nil")
	}
	// auth cache key is the host, repo, and method, joined with a colon separator
	host := req.URL.Host
	repo := ""
	if req.URL.Path != "/v2/" {
		pathMatch := reRepo.FindStringSubmatch(req.URL.Path)
		if len(pathMatch) < 2 {
			return "", fmt.Errorf("could not extract repo from path")
		}
		repo = pathMatch[1]
	}
	// compatible methods are merged
	method := req.Method
	switch method {
	case "GET", "HEAD":
		method = "pull"
	...
	return strings.Join([]string{host, repo, method}, ":"), nil
}
