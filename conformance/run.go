	...
	return r.ChildRun("manifest-by-digest", parent, func(r *runner, res *results) error {
		...
		if len(expectTags) > 0 {
			...
			for _, tag := range expectTags {
				if slices.ContainsFunc(resp.Header.Values("OCI-Tag"), func(v string) bool {
					return slices.ContainsFunc(strings.Split(v, ","), func(s string) bool {
		...
		for subj, referrerGoal := range r.State.Data[tdName].referrers {
			...
			for referrerAT := range referrerATs {
				...
				if err == nil {
					...
					for filter := range strings.SplitSeq(filtersHeader, ",") {
					...
					if filtersApplied {
						...
						for _, resp := range referrerResp.Manifests {
							if resp.ArtifactType != referrerAT {
								errs = append(errs, fmt.Errorf("referrers filter for artifactType %s included descriptor %v%.0w", referrerAT, resp, errAPITestError))
							}
						}
					}
				}
